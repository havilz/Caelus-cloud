...
├── Makefile                            # Perintah otomatisasi (build, run, migrate, test)
│
...
│       └── loki-config.yml
│
└── docs/                               # Dokumentasi Teknis Internal (Git Ignored)
    ├── PROJECT.md                      # Spesifikasi ruang lingkup & domain
    ├── RULES.md                        # Standar kode & alur pengerjaan
...
agent/
├── cmd/
│   └── main.go                         # Entry point daemon caelus-agent
├── internal/
│   ├── collector/                      # Pengumpul metrik host lokal (CPU, RAM, Disk, Net)
│   ├── docker/                         # Klien komunikasi ke Docker socket (/var/run/docker.sock)
│   └── transport/                      # Pengirim payload metrik terenkripsi ke caelus-api
├── go.mod
└── go.sum
```
