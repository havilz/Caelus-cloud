...
## 8. Strategi Lingkungan & Staging (Environments Strategy)

Untuk mendukung fleksibilitas pengembangan tanpa ketergantungan Docker/WSL di perangkat lokal, arsitektur dibagi menjadi dua lingkungan (*environments*):

| Komponen | Development (Dev / Free-Tier) | Production (Prod / Enterprise) |
| :--- | :--- | :--- |
| **Runtime Execution** | Native OS (Go binary + Node.js/pnpm) | Docker & Docker Compose / Containerized VPS |
| **Primary Database** | **Supabase** (Managed PostgreSQL Free-Tier) | Dedicated PostgreSQL (Self-Hosted / Managed Cloud) |
| **Cache & Task Queue** | **Upstash Redis** (Serverless Free-Tier) | Dedicated Redis Cluster |
| **Object Storage** | **Supabase Storage / Cloudflare R2** (Free-Tier) | AWS S3 / Cloudflare R2 / MinIO Cluster |
| **Infrastructure Target** | **MockProvider** (Simulasi In-Memory) | Real Cloud API (AWS, Hetzner, DigitalOcean, VPS) |
| **Observability** | Structured Console Logger + Local Trace | Prometheus + Grafana Loki Stack |

### 8.1 Lingkungan Development (Dev)
- **Tujuan**: Memungkinkan pengembangan dan pengujian fungsional berjalan 100% lancar di perangkat lokal tanpa beban resource dan tanpa kebutuhan instalasi Docker/WSL.
- **Koneksi Database**: Menggunakan *connection string* PostgreSQL dari Supabase yang diinjeksikan melalui file `.env`.
- **Pengujian Fitur**: Menggunakan `MockProvider` untuk mensimulasikan status VPS, Docker container, dan operasi server tanpa memerlukan server eksternal nyata.

### 8.2 Lingkungan Production (Prod)
- **Tujuan**: Menjalankan platform Caelus Cloud di server Linux terpusat dengan performa tinggi, isolasi jaringan, dan reliabilitas tinggi.
- **Deployment**: Menggunakan konfigurasi `deploy/docker/` dan `docker-compose.yml` yang diorkestrasi langsung di server production.